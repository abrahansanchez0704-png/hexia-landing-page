import { useRef, useEffect, useState, createElement, useMemo, useCallback, memo, type RefObject, type MutableRefObject } from 'react';
...
export default function VaporizeTextCycle({
  ...
  useEffect(() => {
    if (onDone) doneCalledRef.current = false;
  }, [onDone]);
  ...
  useEffect(() => {
    ...
    animationState, texts.length, direction, globalDpr,
    memoizedUpdateParticles, memoizedRenderParticles,
    animationDurations.VAPORIZE_DURATION, animationDurations.FADE_IN_DURATION,
    animationDurations.WAIT_DURATION, onDone,
  ]);
...
const renderCanvas = ({
  texts, font, color, alignment,
  canvasRef, wrapperSize, particlesRef, globalDpr, currentTextIndex, transformedDensity,
}: {
  texts: string[];
  ...
  const c = parseColor(color ?? "rgb(153, 153, 153)");

  let textX;
  const textY = canvas.height / 2;
  const currentText = texts[currentTextIndex] || "HEXIA";
  ...
  const { particles, textBoundaries } = createParticles(ctx, canvas, currentText, textX, textY, f, c, alignment);
...
const createParticles = (
  ...
  try { (ctx as any).fontKerning = "normal"; } catch {}
  try { (ctx as any).textRendering = "geometricPrecision"; } catch {}
