...
function upsertClient(db, name, email, countryCode, phone, company, role, industry) {
  const existing = db.prepare('SELECT id FROM clients WHERE email = ?').get(email);
  if (existing) {
    db.prepare("UPDATE clients SET name=?, phone=?, country_code=?, company=?, role=?, industry=?, updated_at=datetime('now') WHERE id=?")
      .run(name, phone||'', countryCode||'', company||'', role||'', industry||'', existing.id);
    return existing.id;
  }
  const r = db.prepare('INSERT INTO clients (name, email, phone, country_code, company, role, industry, source) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    .run(name, email, phone||'', countryCode||'', company||'', role||'', industry||'', 'project');
...
router.post('/newsletter', (req, res) => {
  ...
  const db = getDb();
  const stmt = db.prepare('INSERT INTO newsletter_submissions (email) VALUES (?)');
...
router.post('/projects', (req, res) => {
  ...
  upsertClient(db, name.trim(), email.trim(), countryCode, phone, company, role, industry);
...
router.post('/tickets', async (req, res) => {
  ...
  const result = db.prepare(`INSERT INTO support_tickets (client_name, client_email, client_phone, company, subject, description, priority) VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .run(client_name, client_email, client_phone||'', company||'', subject, description, priority||'normal');
